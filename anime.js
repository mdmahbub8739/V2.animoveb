(function (global) {
  ...
  // ── Anikoto API (recent anime + direct embeds) ─────────────────────────────
  const ANIKOTO_BASE = 'http://anikotoapi.site';
  const _anikotoCache = new Map();
  ...
  // ── Anikoto fetch helpers ───────────────────────────────────────────────────
  async function fetchAnikotoRecent(page = 1, perPage = 16) {
    const key = `recent:${page}:${perPage}`;
    if (_anikotoCache.has(key)) return _anikotoCache.get(key);
    const p = fetch(`${ANIKOTO_BASE}/recent-anime?page=${page}&per_page=${perPage}`)
      .then(r => { if (!r.ok) throw new Error('Anikoto ' + r.status); return r.json(); })
      .then(j => j && j.ok ? j : Promise.reject(new Error('Anikoto bad response')))
      .catch(err => { _anikotoCache.delete(key); throw err; });
    _anikotoCache.set(key, p);
    return p;
  }
  ...
  async function fetchAnikotoSeries(id) {
    const key = `series:${id}`;
    if (_anikotoCache.has(key)) return _anikotoCache.get(key);
    const p = fetch(`${ANIKOTO_BASE}/series/${id}`)
      .then(r => { if (!r.ok) throw new Error('Anikoto ' + r.status); return r.json(); })
      .then(j => j && j.ok ? j : Promise.reject(new Error('Anikoto bad response')))
      .catch(err => { _anikotoCache.delete(key); throw err; });
    _anikotoCache.set(key, p);
  ...
  // ── Anikoto card (different shape: { id, title, poster, ... }) ─────────────
  function anikotoTitleOf(a) { return a.title || a.name || a.title_romaji || a.title_english || 'Unknown'; }
  function anikotoPosterOf(a) { return a.poster || a.image || a.cover || a.thumbnail || ''; }

  function anikotoCard(a) {
    const poster = anikotoPosterOf(a);
    const title = anikotoTitleOf(a);
    const sub = a.type || a.status || '';
    const art = poster
      ? `<img src="${poster}" alt="${html(title)}" loading="lazy" decoding="async">`
      : `<div class="title-card__placeholder"><i class="ri-tv-2-line"></i></div>`;
    return `<a class="title-card" href="#" data-anikoto="${a.id}" data-poster="${html(poster)}" data-title="${html(title)}">
      <div class="title-card__poster"><div class="title-card__shimmer"></div>${art}<span class="title-card__badge">New</span></div>
      <div class="title-card__meta">
        <div class="title-card__title">${html(title)}</div>
        <div class="title-card__sub">${html(sub) || '—'}</div>
      </div>
    </a>`;
  }
  ...
  // ── Anikoto player modal (self-contained, doesn't touch player.js) ─────────
  function injectAnikotoPlayerStyles() {
    if ($('#anikotoPlayerStyles')) return;
    const style = document.createElement('style');
    style.id = 'anikotoPlayerStyles';
    style.textContent = `
      .anikoto-modal { position:fixed; inset:0; z-index:9999; display:flex; flex-direction:column;
        background:#000; opacity:0; pointer-events:none; transition:opacity .22s ease; }
      .anikoto-modal.is-visible { opacity:1; pointer-events:auto; }
      .anikoto-modal__top { display:flex; align-items:center; justify-content:space-between;
        padding:12px 16px; background:rgba(10,10,10,0.92); flex:0 0 auto; gap:12px; }
      .anikoto-modal__title { font-size:14px; font-weight:600; color:#fff; overflow:hidden;
        text-overflow:ellipsis; white-space:nowrap; }
      .anikoto-modal__close { background:rgba(255,255,255,0.08); border:none; color:#fff;
        width:34px; height:34px; border-radius:50%; font-size:18px; cursor:pointer; flex:0 0 auto; }
      .anikoto-modal__close:hover { background:rgba(255,255,255,0.18); }
      .anikoto-modal__stage { flex:1 1 auto; position:relative; background:#000; }
      .anikoto-modal__stage iframe { position:absolute; inset:0; width:100%; height:100%; border:0; }
      .anikoto-modal__bar { display:flex; gap:8px; padding:10px 16px; background:rgba(10,10,10,0.92);
        flex:0 0 auto; overflow-x:auto; }
      .anikoto-modal__ep, .anikoto-modal__lang { background:rgba(255,255,255,0.08); border:none;
        color:#ddd; padding:7px 14px; border-radius:99px; font-size:12.5px; cursor:pointer; white-space:nowrap; }
      .anikoto-modal__ep.active, .anikoto-modal__lang.active { background:var(--accent,#e50914); color:#fff; }
      .anikoto-modal__msg { color:#aaa; padding:40px 16px; text-align:center; }
    `;
    document.head.appendChild(style);
  }

  function closeAnikotoPlayer() {
    const m = $('#anikotoModal');
    if (!m) return;
    m.classList.remove('is-visible');
    setTimeout(() => { m.querySelector('.anikoto-modal__stage').innerHTML = ''; m.remove(); }, 220);
  }

  function buildAnikotoModal(title) {
    closeAnikotoPlayer();
    injectAnikotoPlayerStyles();
    const m = document.createElement('div');
    m.id = 'anikotoModal';
    m.className = 'anikoto-modal';
    m.innerHTML = `
      <div class="anikoto-modal__top">
        <div class="anikoto-modal__title">${html(title || '')}</div>
        <button class="anikoto-modal__close" aria-label="Close">✕</button>
      </div>
      <div class="anikoto-modal__stage"><div class="anikoto-modal__msg">Loading…</div></div>
      <div class="anikoto-modal__bar" id="anikotoEpBar"></div>
    `;
    document.body.appendChild(m);
    m.querySelector('.anikoto-modal__close').addEventListener('click', closeAnikotoPlayer);
    m.addEventListener('click', (e) => { if (e.target === m) closeAnikotoPlayer(); });
    requestAnimationFrame(() => m.classList.add('is-visible'));
    return m;
  }

  function playInAnikotoModal(embedUrl) {
    const stage = $('#anikotoModal .anikoto-modal__stage');
    if (!stage) return;
    if (!embedUrl) { stage.innerHTML = '<div class="anikoto-modal__msg">No stream available for this episode.</div>'; return; }
    stage.innerHTML = `<iframe src="${embedUrl}" allowfullscreen allow="autoplay; fullscreen; encrypted-media; picture-in-picture"></iframe>`;
  }

  async function openAnikotoPlayer(seriesId, poster, title) {
    showResolve(poster, title);
    try {
      const data = await fetchAnikotoSeries(seriesId);
      hideResolve();
      const series = data.anime || data.series || data;
      const episodes = data.episodes || series.episodes || [];
      buildAnikotoModal(anikotoTitleOf(series) || title);

      if (!episodes.length) {
        playInAnikotoModal(null);
        return;
      }

      const epBar = $('#anikotoEpBar');
      let currentEp = episodes[0];
      let currentLang = currentEp.embed_url?.sub ? 'sub' : 'dub';

      function renderLangButtons() {
        const langs = [];
        if (currentEp.embed_url?.sub) langs.push('sub');
        if (currentEp.embed_url?.dub) langs.push('dub');
        return langs.map(l => `<button class="anikoto-modal__lang ${l === currentLang ? 'active' : ''}" data-lang="${l}">${l.toUpperCase()}</button>`).join('');
      }
      ...
      function renderEpButtons() {
        return episodes.map((ep, i) => {
          ...
          return `<button class="anikoto-modal__ep ${active ? 'active' : ''}" data-idx="${i}">Ep ${num}</button>`;
        }).join('');
      }

      function paintBar() {
        epBar.innerHTML = renderLangButtons() + renderEpButtons();
      }

      function playCurrent() {
        const url = currentEp.embed_url?.[currentLang] || currentEp.embed_url?.sub || currentEp.embed_url?.dub || null;
        playInAnikotoModal(url);
        paintBar();
      }

      epBar.addEventListener('click', (e) => {
        const langBtn = e.target.closest('[data-lang]');
        const epBtn = e.target.closest('[data-idx]');
        if (langBtn) {
          currentLang = langBtn.dataset.lang;
          playCurrent();
        } else if (epBtn) {
          currentEp = episodes[+epBtn.dataset.idx];
          currentLang = currentEp.embed_url?.sub ? 'sub' : 'dub';
          playCurrent();
        }
      });
  ...
  function wireCards(root) {
    root.addEventListener('click', (e) => {
      const akCard = e.target.closest('.title-card[data-anikoto]');
      if (akCard) {
        e.preventDefault();
        openAnikotoPlayer(akCard.dataset.anikoto, akCard.dataset.poster || '', akCard.dataset.title || '');
        return;
      }
  ...
  // ── Anikoto rail paint (separate card shape, separate pagination) ──────────
  function paintAnikotoRail(view, sel, items) {
    const sec = view.querySelector(sel);
    if (!sec) return;
    const rail = sec.querySelector('.rail');
    if (!rail) return;
    if (!items?.length) { rail.outerHTML = '<p class="an-empty">No titles found.</p>'; return; }
    rail.innerHTML = `<div class="rail__strip">${items.map(anikotoCard).join('')}</div>`;
    setupAnikotoRailInfinite(sec);
  }

  function setupAnikotoRailInfinite(sec) {
    ...
    const io = new IntersectionObserver(async (entries) => {
      ...
      try {
        page += 1;
        const data = await fetchAnikotoRecent(page, 16);
        const items = data.data || data.results || data.animes || [];
        if (items.length) {
          const frag = document.createElement('div');
          frag.innerHTML = items.map(anikotoCard).join('');
          [...frag.children].forEach(c => strip.insertBefore(c, sentinel));
        }
        const hasNext = data.pagination?.has_next_page ?? (items.length > 0);
        if (!hasNext) {
          exhausted = true;
  ...
  async function renderPage(view) {
    ...
    view.innerHTML = `
      ${skHero()}
      ${section('an-recent',   'Recently <em>updated</em>', 'Fresh episodes, straight from Anikoto.', 'recent', null)}
    ...
    fetchAnikotoRecent(1, 16).then(data => {
      if (!view.isConnected) return;
      const items = data.data || data.results || data.animes || [];
      paintAnikotoRail(view, '#an-recent', items);
    }).catch(() => {
      const sec = view.querySelector('#an-recent');
  ...
  global.AnikotoModule = { fetchAnikotoRecent, fetchAnikotoSeries, openAnikotoPlayer };
})(window);
