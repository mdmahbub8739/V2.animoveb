...
(function(global) {
  ...
  const O_SS = 'https://screenscape.me';
  const O_A = 'https://web.nxsha.app';
  const O_B = 'https://cinemaos.tech';
  const O_C = 'https://peachify.top';
  const O_VR = 'https://vidrock.ru';

  const TRUSTED_ORIGINS = [O_SS, O_A, O_B, O_C, O_VR];
  const PROGRESS_STORAGE_KEY = 'peachifyProgress';
  ...
  function readProgressStore() {
    try { return JSON.parse(localStorage.getItem(PROGRESS_STORAGE_KEY) || '{}'); } catch (_) { return {}; }
  }
  ...
  function buildScreenscapeUrl(ctx, opts = {}) {
    ...
    if (typeof ctx.id === 'string' && /^tt\d+/.test(ctx.id)) p.set('imdb', ctx.id);
    else p.set('tmdb', String(ctx.id));
    p.set('type', ctx.type);
    if (ctx.type === 'tv') {
  ...
  function buildNebulaUrl(ctx, opts = {}) {
    ...
    p.set('title', 'false'); p.set('poster', 'false');
    if (ctx.type === 'tv') {
  ...
  class StellarPlayer {
    constructor(target, options = {}) {
      ...
      this.opts = Object.assign({ accent: null, autoPlay: true, autoNext: true, showNextBtn: true, hide: null, servers: defaultChain(), onEvent: null, onProgress: null, onSourceChange: null, onLoading: null }, options || {});
      this.ctx = null; this.serverIndex = 0; this._iframe = null;
      this._installListener();
    }
    playMovie(id, perCallOpts) {
      if (!validId(id)) { console.warn('StellarPlayer: invalid id', id); return false; }
      this.ctx = { type: 'movie', id, _opts: perCallOpts || {} }; this.serverIndex = 0; this._mount(); return true;
    }
    playEpisode(id, season, episode, perCallOpts) {
      if (!validId(id)) { console.warn('StellarPlayer: invalid id', id); return false; }
      this.ctx = { type: 'tv', id, season, episode, _opts: perCallOpts || {} }; this.serverIndex = 0; this._mount(); return true;
    }
    next() { this._rotate('manual next()'); }
    setSource(i) { if (i < 0 || i >= this.opts.servers.length) return; this.serverIndex = i; this._mount(); }
    listSources() { return this.opts.servers.map((s, i) => ({ index: i, name: s.name })); }
    currentSourceName() { const s = this.opts.servers[this.serverIndex]; return s ? s.name : null; }
    destroy() { this.host.innerHTML = ''; this._iframe = null; window.removeEventListener('message', this._onMessage); }
    ...
    _mount() {
      ...
      if (!srv) return;
      const merged = Object.assign({ accent: this.opts.accent, autoPlay: this.opts.autoPlay, autoNext: this.opts.autoNext, showNextBtn: this.opts.showNextBtn, hide: this.opts.hide }, srv.opts || {}, this.ctx._opts || {});
      let url;
      ...
      this._iframe = ifr; this.host.appendChild(ifr);
    }
    _rotate(reason) {
      const next = this.serverIndex + 1;
      if (next >= this.opts.servers.length) {
        console.warn('[StellarPlayer] All sources exhausted —', reason);
        if (typeof this.opts.onLoading === 'function') this.opts.onLoading(false, this.currentSourceName(), 'exhausted');
        return;
      }
      this.serverIndex = next;
      console.info('[StellarPlayer] Manually switching to', this.opts.servers[next].name, '—', reason);
      this._mount();
    }
    _installListener() {
      this._onMessage = (event) => {
        ...
        if (msg.type === 'MEDIA_DATA' && msg.data) {
          ...
          if (Array.isArray(msg.data)) {
            msg.data.forEach((rec) => { if (!rec || rec.id == null) return; store[String(rec.id)] = rec; });
          } else {
            Object.keys(msg.data).forEach(k => { const rec = msg.data[k]; if (!rec) return; const key = rec.id != null ? String(rec.id) : String(k).replace(/^m/, ''); store[key] = rec; });
          }
