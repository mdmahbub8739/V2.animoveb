...
/* ═══════════════════════════════════════════════════════════════════════════
 *  MegaPlayPlayer — Anime-specific embed player via MegaPlay / Anikoto
 *
 *  Simpler than StellarPlayer — single source (MegaPlay), no provider chain.
 *  Supports:
 *    • play(episodeEmbedId, language)     — Anikoto episode embed ID
 *    • playByAniList(anilistId, epNum, language)
 *    • playByMAL(malId, epNum, language)
 *    • postMessage event handling (progress, complete, watch-log)
 *
 *  Usage:
 *    const ap = new MegaPlayPlayer('#playerFrame');
 *    ap.play('136197', 'sub');
 *    ap.onComplete = () => { /* load next ep */ };
 * ═══════════════════════════════════════════════════════════════════════════ */
(function (global) {
  'use strict';

  const _CFG = global.OMNIFLIX_CONFIG || {};
  const MEGAPLAY_BASE = _CFG.MEGAPLAY_BASE || 'https://megaplay.buzz';

  class MegaPlayPlayer {
    constructor(target, options = {}) {
      this.host = (typeof target === 'string') ? document.querySelector(target) : target;
      if (!this.host) throw new Error('MegaPlayPlayer: target element not found');

      this.opts = Object.assign({
        onProgress: null,   // (data) => {} — time/progress events
        onComplete: null,   // () => {} — episode finished
        onWatchLog: null,   // (data) => {} — watching-log events
        onEvent: null,      // (data) => {} — any megacloud event
      }, options);
    ...
    // ── Play by Anikoto episode_embed_id ──────────────────────────────────
    play(episodeEmbedId, language) {
      const lang = language || 'sub';
      this.currentEmbedId = episodeEmbedId;
      this.currentLanguage = lang;
      const url = `${MEGAPLAY_BASE}/stream/s-2/${episodeEmbedId}/${lang}`;
      this._mount(url);
      return true;
    }

    // ── Play by AniList ID + episode number ───────────────────────────────
    playByAniList(anilistId, epNum, language) {
      const lang = language || 'sub';
      this.currentLanguage = lang;
      const url = `${MEGAPLAY_BASE}/stream/ani/${anilistId}/${epNum}/${lang}`;
      this._mount(url);
      return true;
    }

    // ── Play by MAL ID + episode number ───────────────────────────────────
    playByMAL(malId, epNum, language) {
      const lang = language || 'sub';
      this.currentLanguage = lang;
      const url = `${MEGAPLAY_BASE}/stream/mal/${malId}/${epNum}/${lang}`;
      this._mount(url);
      return true;
    }

    // ── Switch language without reloading page ────────────────────────────
    switchLanguage(lang) {
      if (this.currentEmbedId) {
        this.play(this.currentEmbedId, lang);
      }
    }
    ...
    _mount(url) {
      ...
      ifr.style.cssText = 'width:100%;height:100%;border:0;display:block;background:#000;';
      ifr.setAttribute('allowfullscreen', '');
      ifr.setAttribute('allow', 'autoplay; encrypted-media; fullscreen; clipboard-write');
    ...
    _installListener() {
      this._onMessage = (event) => {
        ...
        // MegaCloud / MegaPlay channel events
        if (data.channel === 'megacloud' || data.event) {
          if (typeof this.opts.onEvent === 'function') this.opts.onEvent(data);
          ...
          if (data.event === 'time') {
            if (typeof this.opts.onProgress === 'function') this.opts.onProgress(data);
          }
        }

        // Watching log
        if (data.type === 'watching-log') {
          if (typeof this.opts.onWatchLog === 'function') this.opts.onWatchLog(data);
        }
      };
      window.addEventListener('message', this._onMessage);
    }
  ...
  // Expose
  global.MegaPlayPlayer = MegaPlayPlayer;
})(typeof window !== 'undefined' ? window : globalThis);
