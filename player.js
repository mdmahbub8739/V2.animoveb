...
(function(global) {
    ...
    const O_SS = 'https://screenscape.me'; // ScreenScape

    const TRUSTED_ORIGINS = [O_A, O_B, O_C, O_VR, O_SS];
    ...
    // Safely normalizes variations of language parameters based on provider requirements
    function getCleanLang(kind, opts) {
        const raw = opts.lan || opts.lang || opts.dub || 'hi';
        const normalized = String(raw).toLowerCase();

        if (kind === 'aurora') {
            if (normalized.includes('hin') || normalized === 'hi') return 'hi';
            if (normalized.includes('eng') || normalized === 'en') return 'en';
            return normalized;
        }
        if (kind === 'peachify') {
            if (normalized.includes('hin') || normalized === 'hi') return 'Hindi';
            if (normalized.includes('eng') || normalized === 'en') return 'English';
            return raw;
        }
        if (kind === 'screenscape') {
            if (normalized.includes('hin') || normalized === 'hi') return 'hindi';
            if (normalized.includes('eng') || normalized === 'en') return 'eng';
            return normalized;
        }
        return raw;
    }
    ...
    function buildAuroraUrl(ctx, opts = {}) {
        ...
        p.set('lang', getCleanLang('aurora', opts));
    ...
    function buildPeachifyUrl(ctx, opts = {}) {
        ...
        const p = new URLSearchParams();
        p.set('dub', getCleanLang('peachify', opts));
    ...
    function buildScreenscapeUrl(ctx, opts = {}) {
        ...
        p.set('lan', getCleanLang('screenscape', opts));
    ...
    class StellarPlayer {
        ...
        listSources() {
            return this.opts.servers.map((s, i) => ({
                index: i,
                name: s.name
            }));
        }
        ...
        _mount() {
            ...
            else if (srv.kind === 'screenscape') url = buildScreenscapeUrl(this.ctx, merged);
